...
import { BookOpen, Filter, Search, ShieldCheck, Sparkles, X } from "lucide-react";
...
const RoomsPage = () => {
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      const roomName = String(room.roomName || room.title || room.name || "").toLowerCase();
      const roomDescription = String(room.description || room.summary || room.subtitle || "").toLowerCase();
      ...
      const matchesAmenities = selectedAmenities.every((amenity) => amenities.includes(amenity));
      const rate = Number(room.hourlyRate ?? room.price ?? room.rate ?? 0);
      const matchesMin = minPrice === "" || rate >= minValue;
      const matchesMax = maxPrice === "" || rate <= maxValue;

      return matchesSearch && matchesAmenities && matchesMin && matchesMax;
    });
  }, [rooms, searchValue, selectedAmenities, minPrice, maxPrice, minValue, maxValue]);

  const activeFilters =
    Number(searchValue.length > 0) + selectedAmenities.length + Number(minPrice !== "") + Number(maxPrice !== "");
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="mb-10 overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/75 p-8 shadow-2xl shadow-slate-950/20 backdrop-blur-xl">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl space-y-4">
              <p className="inline-flex items-center gap-2 rounded-full bg-slate-800/80 px-4 py-2 text-sm uppercase tracking-[0.35em] text-sky-300 shadow-inner shadow-slate-950/20">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:w-96">
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5 shadow-xl shadow-slate-950/20">
                <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Rooms loaded</p>
                <p className="mt-3 text-3xl font-semibold text-white">{rooms.length}</p>
              </div>
              <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5 shadow-xl shadow-slate-950/20">
                <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Filters active</p>
        ...
        <div className="grid gap-8 xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="hidden xl:block">
            <div className="sticky top-28 space-y-6 rounded-[2rem] border border-white/10 bg-slate-900/80 p-6 shadow-2xl shadow-slate-950/20 backdrop-blur-xl">
              <div className="space-y-3">
                <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Refine search</p>
                <h2 className="text-2xl font-semibold text-white">Filter rooms</h2>
                <p className="text-sm leading-6 text-slate-400">
                  Toggle amenities, adjust rate range, or search by room name.
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5">
                  <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Amenities</p>
                ...
                <div className="grid gap-4 rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5">
                  <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Price range</p>
          ...
          <section className="space-y-8">
            <div className="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-slate-950/20 backdrop-blur-xl lg:flex-row lg:justify-between lg:items-center">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Results</p>
                <h2 className="mt-2 text-3xl font-semibold text-white">{filteredRooms.length} rooms found</h2>
              </div>
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                >
                  ...
                  <h3 className="text-2xl font-semibold text-white">No rooms match your filters</h3>
                  <p className="mt-3 text-sm leading-6 text-slate-400">
                    Try broadening the price range or removing a selected amenity to reveal more study rooms.
                  </p>
                  <button
                    type="button"
                    onClick={resetFilters}
        ...
        {drawerOpen ? (
          ...
          >
            <motion.aside
              ...
              className="absolute right-0 top-0 h-full w-full max-w-sm border-l border-white/10 bg-slate-950/95 p-6 shadow-2xl shadow-slate-950/50"
            >
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Filters</p>
                  <h2 className="text-xl font-semibold text-white">Refine rooms</h2>
                </div>
                <button
                  type="button"
                  onClick={() => setDrawerOpen(false)}
                  aria-label="Close filters"
                  className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-slate-900/80 text-slate-200 transition hover:border-sky-300/40"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="mt-8 space-y-6">
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(event) => setSearchTerm(event.target.value)}
                    placeholder="Search rooms"
                    className="w-full rounded-3xl border border-white/10 bg-slate-950/80 px-12 py-3 text-sm text-slate-100 outline-none transition focus:border-sky-400/50 focus:ring-2 focus:ring-sky-400/20"
                  />
                </div>

                <div className="space-y-3 rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-5">
                  <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Amenities</p>
                  <div className="grid gap-3">
                    {amenityOptions.map((amenity) => (
                      <label
                ...
                <div className="grid gap-4 rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-5">
                  <p className="text-sm uppercase tracking-[0.28em] text-slate-400">Price range</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="space-y-2 text-sm text-slate-300">
                      <span>Min</span>
